...
public class WarehouseNPC : MonoBehaviour
{
    private Text promptText;

    void Awake()
    {
        GameObject canvas = UIFactory.CreateCanvas("WarehousePromptCanvas", 44);
        promptText = UIFactory.CreateText(canvas.transform, "WarehousePrompt", "", 16, TextAnchor.MiddleCenter,
    ...
    void Update()
    {
        ...
        {
            promptText.text = "Pulsa E para usar el Almacén";
            if (Input.GetKeyDown(KeyCode.E) && !WarehouseUI.IsOpen) WarehouseUI.Toggle();
        }
        else
        {
            promptText.text = "";
